...
func RenderUserRow(w io.Writer, u User) {
	...
	io.WriteString(w, `<tr><td class="px-6 py-4 whitespace-nowrap"><div class="flex items-center"><div class="flex-shrink-0 h-10 w-10"><div class="h-10 w-10 rounded-full bg-blue-600 flex items-center justify-center text-white font-medium text-sm">`+u.Avatar+`</div></div><div class="ml-4"><div class="text-sm font-medium text-gray-900">`+u.Name+`</div><div class="text-sm text-gray-500">`+u.Email+`</div></div></div></td><td class="px-6 py-4 whitespace-nowrap"><span class="px-2 inline-flex text-xs leading-5 font-semibold rounded-full bg-gray-100 text-gray-800">`+u.Role+`</span></td><td class="px-6 py-4 whitespace-nowrap">`+statusBadge+`</td><td class="px-6 py-4 whitespace-nowrap text-sm text-gray-500"><button class="text-blue-600 hover:text-blue-900 mr-3" hx-get="/api/users/edit?id=`+strconv.Itoa(u.ID)+`" hx-target="#modal-container" hx-swap="innerHTML">Editar</button><button class="text-red-600 hover:text-red-900" hx-delete="/api/users?id=`+strconv.Itoa(u.ID)+`" hx-target="closest tr" hx-swap="delete" hx-confirm="Tem certeza que deseja excluir este usuario?">Excluir</button></td></tr>`)
}
