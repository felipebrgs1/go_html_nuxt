...
func Generate(root string, routes []router.Route) error {
	...
	for _, r := range routes {
		...
		pattern := r.Pattern
		// Rota raiz usa /{$} para nao capturar rotas nao encontradas como fallback
		if pattern == "/" {
			pattern = "/{$}"
		}
		line := fmt.Sprintf("\tmux.HandleFunc(\"%s %s\", %s.%s)", r.Method, pattern, alias, r.HandlerName)
		regLines = append(regLines, line)
	}

	// Handler catch-all para rotas nao encontradas
	regLines = append(regLines, "\tmux.HandleFunc(\"GET /{path...}\", NotFound)")
